...
exports.createCourse = async (req, res) => {
  try {
    ...
    const parsedLearn = Array.isArray(whatYouWillLearn)
      ? whatYouWillLearn
    ...
    const course = new Course({
      ...
      certificate: certificate !== undefined ? Boolean(certificate) : true,
...
exports.uploadThumbnail = async (req, res) => {
  try {
    if (!req.file) {
      return res.status(400).json({ success: false, message: 'No thumbnail image file uploaded.' });
    }
    ...
    return res.status(500).json({ success: false, message: 'Server error during thumbnail upload.' });
  }
};
...
exports.uploadLessonResource = async (req, res) => {
  try {
    ...
    const fileExt = req.file.filename.split('.').pop().toUpperCase();
    const fileSizeMb = (req.file.size / (1024 * 1024)).toFixed(1) + ' MB';
    const fileUrl = `http://localhost:5000/uploads/resources/${req.file.filename}`;
